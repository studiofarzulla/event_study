...
class EventImpactAnalysis:
                ...
                if tarchx.event_effects:
                    for event_var, coef in tarchx.event_effects.items():
                        ...
                        if event_var == "D_infrastructure":
                            ...
                            event_var.startswith("S_")
                            or "gdelt_normalized" in event_var
                            or "reg_decomposed" in event_var
                            or "infra_decomposed" in event_var
                        ):
    ...
    def test_infrastructure_vs_regulatory(self) -> Dict:
        ...
        if len(infra_coefs) > 0 and len(reg_coefs) > 0:
            ...
            pooled_std = np.sqrt(
                ((len(infra_coefs) - 1) * np.var(infra_coefs) + (len(reg_coefs) - 1) * np.var(reg_coefs))
            ...
            print(
                f"  Result:      {'Reject H0 ***' if t_pval < 0.01 else 'Reject H0 **' if t_pval < 0.05 else 'Reject H0 *' if t_pval < 0.10 else 'Fail to reject H0'}"
            )
            ...
            print(
                f"  Result:      {'Reject H0 ***' if u_pval < 0.01 else 'Reject H0 **' if u_pval < 0.05 else 'Reject H0 *' if u_pval < 0.10 else 'Fail to reject H0'}"
            )

            print(f"\nEffect size (Cohen's d): {cohens_d:.4f}")
            print(
                f"  Interpretation: {'Large' if abs(cohens_d) > 0.8 else 'Medium' if abs(cohens_d) > 0.5 else 'Small' if abs(cohens_d) > 0.2 else 'Negligible'}"
            )
    ...
    def apply_fdr_correction(self) -> pd.DataFrame:
        ...
        rejected, corrected_pvals = fdrcorrection(valid_pvals["p_value"].values, alpha=self.fdr_alpha)
        ...
        if n_sig_fdr > 0:
            ...
            for _, row in sig_events.iterrows():
                print(
                    f"  {row['crypto']:5} {row['event_variable']:25} "
    ...
    def analyze_by_cryptocurrency(self) -> pd.DataFrame:
            ...
            summary_data.append(
                {
                    ...
                    "difference": (
                        (infra["coefficient"].mean() - reg["coefficient"].mean())
                        if len(infra) > 0 and len(reg) > 0
                        else np.nan
                    ),
                }
            )
        ...
        if not summary_df.empty:
            ...
            print("-" * 60)
            print(
                f"{'Crypto':<8} {'Infra Mean':<12} {'Reg Mean':<12} {'Difference':<12} {'Sig Infra':<10} {'Sig Reg':<10}"
            )
            print("-" * 60)
                    ...
                    pub_data.append(
                        {
                            ...
                            "Sig (FDR)": (
                                type_events["fdr_significant"].sum() if "fdr_significant" in type_events else 0
                            ),
                        }
                    )
    ...
    def calculate_inverse_variance_weighted_average(self) -> Dict:
        ...
        if "Infrastructure" in results and "Regulatory" in results:
            ...
            se_diff = np.sqrt(
                results["Infrastructure"]["standard_error"] ** 2 + results["Regulatory"]["standard_error"] ** 2
            )
            z_stat = diff / se_diff
            ...
            print(
                f"  Significant:    {'Yes***' if p_value < 0.01 else 'Yes**' if p_value < 0.05 else 'Yes*' if p_value < 0.10 else 'No'}"
            )
    ...
    def test_sentiment_volatility_relationship(
        self, crypto_data: Dict, sentiment_data: Optional[pd.DataFrame] = None
    ) -> Dict:
        """
                ...
                if "S_reg_decomposed" in df.columns and "S_infra_decomposed" in df.columns:
                    ...
                    for lag in lags:
                        ...
                        if len(aligned) > 10:
                            ...
                            infra_correlations.append({"lag": lag, "correlation": corr, "p_value": pval})
                    ...
                    for lag in lags:
                        ...
                        if len(aligned) > 10:
                            corr, pval = pearsonr(aligned.iloc[:, 0], aligned.iloc[:, 1])
                            reg_correlations.append({"lag": lag, "correlation": corr, "p_value": pval})

                    # Find optimal lags (maximum absolute correlation)
                    if infra_correlations:
                        optimal_infra = max(infra_correlations, key=lambda x: abs(x["correlation"]))
                    else:
                        optimal_infra = None

                    if reg_correlations:
                        optimal_reg = max(reg_correlations, key=lambda x: abs(x["correlation"]))
                    else:
                        optimal_reg = None
                    ...
                    test_data = pd.concat(
                        [vol_weekly, sent_infra, sent_reg], axis=1, keys=["volatility", "sent_infra", "sent_reg"]
                    ).dropna()
                    ...
                    if len(test_data) > 20:
                        try:
                            ...
                            granger_infra = grangercausalitytests(
                                test_data[["volatility", "sent_infra"]], maxlag=4, verbose=False
                            )
                        ...
                        try:
                            ...
                            granger_reg = grangercausalitytests(
                                test_data[["volatility", "sent_reg"]], maxlag=4, verbose=False
                            )
                    ...
                    crypto_results = {
                        "infrastructure": {
                            ...
                            "interpretation": (
                                "Contemporaneous"
                                if optimal_infra and optimal_infra["lag"] == 0
                                else (
                                    "Sentiment leads"
                                    if optimal_infra and optimal_infra["lag"] < 0
                        ...
                        "regulatory": {
                            ...
                            "interpretation": (
                                "Sentiment leads"
                                if optimal_reg and optimal_reg["lag"] < 0
                                else (
                                    "Contemporaneous" if optimal_reg and optimal_reg["lag"] == 0 else "Volatility leads"
                                )
                            ),
                        },
    ...
    def _evaluate_h2_support(self, optimal_infra: Optional[Dict], optimal_reg: Optional[Dict]) -> str:
        """
    ...
    def calculate_persistence_by_event_type(self) -> Dict:
        ...
        for crypto, models in self.model_results.items():
            ...
            persistence_by_type[crypto] = {
                "base": {"persistence": base_persistence, "half_life": calc_half_life(base_persistence)},
                "infrastructure": {
