...
class HypothesisTestingResults:
    ...
    def load_all_crypto_data(self) -> Dict:
        ...
        self.crypto_data = self.prep.prepare_all_cryptos(include_events=True, include_sentiment=True)

        # Remove any None or empty DataFrames
        self.crypto_data = {k: v for k, v in self.crypto_data.items() if v is not None and not v.empty}
    ...
    def test_hypothesis_3(self) -> Dict:
        ...
        # Calculate win rates
        wins = sum(1 for _, r in comparison_df.iterrows() if "AIC_improvement" in r and r["AIC_improvement"] > 0)
