...
def save_results_to_csv(results: dict, output_dir: Path):
    ...
    if "hypothesis_test" in results:
        ...
        hypothesis_df = pd.concat(
            [hypothesis_df, pd.DataFrame([results["hypothesis_test"]["regulatory"]], index=["Regulatory"])]
        )
...
def main(run_robustness: bool = False, run_bootstrap: bool = False, generate_publication: bool = True):
    ...
    for crypto in all_cryptos:
        ...
        try:
            crypto_data[crypto] = data_prep.prepare_crypto_data(crypto, include_events=True, include_sentiment=True)
            print(f"  [OK] Shape: {crypto_data[crypto].shape}")
            print(
                f"  Returns - Mean: {crypto_data[crypto]['returns_winsorized'].mean():.4f}%, "
    ...
    for crypto in model_results:
        garch_ok = (
            model_results[crypto].get("GARCH(1,1)", {}).convergence if "GARCH(1,1)" in model_results[crypto] else False
        )
        tarch_ok = (
            model_results[crypto].get("TARCH(1,1)", {}).convergence if "TARCH(1,1)" in model_results[crypto] else False
        )
        tarchx_ok = (
            model_results[crypto].get("TARCH-X", {}).convergence if "TARCH-X" in model_results[crypto] else False
        )

        print(
            f"{crypto.upper():5} - GARCH: {'[OK]' if garch_ok else '[FAIL]'} | "
    ...
    if "hypothesis_test" in analysis_results and analysis_results["hypothesis_test"]:
        ...
        if "infrastructure" in hyp_test and "regulatory" in hyp_test:
            ...
            if "t_test" in hyp_test:
                ...
                print(
                    f"Result: {'Infrastructure > Regulatory (significant)' if p_val < 0.10 else 'No significant difference'}"
                )
    ...
    if "hypothesis_test" in analysis_results and "t_test" in analysis_results["hypothesis_test"]:
        ...
        if p_val < 0.01:
            print(
                "*** Infrastructure events have SIGNIFICANTLY LARGER volatility impacts than Regulatory events (p < 0.01) ***"
            )
        elif p_val < 0.05:
            print(
                "** Infrastructure events have SIGNIFICANTLY LARGER volatility impacts than Regulatory events (p < 0.05) **"
            )
        elif p_val < 0.10:
    ...
    if run_robustness:
        ...
        robustness_results = run_robustness_checks(
            cryptos=["btc", "eth"], run_bootstrap=run_bootstrap  # Limited cryptos for speed
        )
    ...
    if run_bootstrap and not run_robustness:  # If not already done in robustness
        ...
        if "btc" in crypto_data:
            ...
            bootstrap_results = run_bootstrap_analysis(btc_returns, model_type="TARCH", n_bootstrap=500)
